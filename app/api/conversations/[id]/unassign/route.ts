...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const previousAgent = conv.assigned_agent_id
  const { error: updErr } = await admin
    .from('conversations')
    .update({ assigned_agent_id: null })
    .eq('id', conversationId)
  if (updErr) return NextResponse.json({ error: 'unassign_failed' }, { status: 400 })

  await admin
    .from('conversation_assignments')
    .insert({ conversation_id: conversationId, agent_id: previousAgent, unassigned_at: new Date().toISOString() })
